...
export class ComplejoController {
  ...
  @Post()
  async create(@Body() createComplejoDto: CreateComplejoDto): Promise<Complejo> {
    return await this.complejoService.create(createComplejoDto);
  }

  @Get()
  async findAll(): Promise<Complejo[]> {
    return await this.complejoService.findAll();
  }
  ...
  @Patch(':id')
  async update(@Param('id') id: number, @Body() updateComplejoDto: UpdateComplejoDto): Promise<Complejo> {
